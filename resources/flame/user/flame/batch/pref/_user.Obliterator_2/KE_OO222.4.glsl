...
// uniform sampler2D adsk_results_pass3; // input
uniform float adsk_result_w, adsk_result_h; // size of the textures
uniform sampler2D adsk_results_pass4; // previous execution result
uniform sampler2D adsk_results_pass3;

// horizontal / vertical size of the blur in pixels
uniform vec2 maxBlurSizeAD;
uniform float propMaxBlurSizeAD;
uniform bool propBlurAD;
...
void main()
{
   ...
   //float currmaxBlurSizeAD = firstExecution ? maxBlurSizeAD.x : maxBlurSizeAD.y;

   float currMaxBlurSizeAD;
   if (propBlurAD)
      currMaxBlurSizeAD = propMaxBlurSizeAD;
   else
   {
      if (firstExecution)
         currMaxBlurSizeAD = maxBlurSizeAD.x;
      else
         currMaxBlurSizeAD = maxBlurSizeAD.y;
   }

   int support = int( ceil( currMaxBlurSizeAD ) );
   float sigma = currMaxBlurSizeAD * 0.4;
   ...
   vec4  frag = firstExecution ?
         texture2D( adsk_results_pass3, uv ) : texture2D( adsk_results_pass4, uv );
   ...
   for( int off = 1; off < support; ++off )
   {
      ...
      vec4 frag_m = firstExecution ? 
         texture2D( adsk_results_pass3, uv - uv_offset ) : texture2D( adsk_results_pass4, uv - uv_offset );
   ...
   if ( (propBlurAD) && (propMaxBlurSizeAD == 0.0) )
      gl_FragColor = texture2D(adsk_results_pass3, uv);
   else if ( (propBlurAD == false) && (currMaxBlurSizeAD == 0.0) && (firstExecution) )
      gl_FragColor = texture2D(adsk_results_pass3, uv);
   else if ( (propBlurAD == false) && (currMaxBlurSizeAD == 0.0) && (firstExecution == false) )
