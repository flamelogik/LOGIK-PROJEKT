...
for cmd in openssl ssh-keygen tar chmod; do
...
# Prompt user to choose a folder using osascript
chosen_folder=$(osascript -e 'POSIX path of (choose folder with prompt "Choose Folder to Save New SSH Keys")')

# Check if user cancelled the folder selection
if [ $? -ne 0 ]; then
    osascript -e 'display alert "Operation cancelled. Exiting." as critical'
    exit 1
fi
...
if [ -z "$email_address" ]; then
    osascript -e 'display alert "Email address cannot be empty. Exiting." as critical'
    exit 1
fi
...
while [ "$password_match" -eq 0 ]; do
    my_password=$(osascript -e 'display dialog "Enter Password" default answer "" with hidden answer' -e 'text returned of result')
    confirm_password=$(osascript -e 'display dialog "Confirm Password" default answer "" with hidden answer' -e 'text returned of result')
    
    if [ "$my_password" = "$confirm_password" ]; then
        password_match=1
    else
        osascript -e 'display alert "Passwords do not match. Please try again." as critical'
    fi
done
...
if [ $? -ne 0 ]; then
    osascript -e 'display alert "Failed to create tar file. Exiting." as critical'
    exit 1
fi
...
if [ $? -ne 0 ]; then
    osascript -e 'display alert "Failed to encrypt tar file. Exiting." as critical'
    exit 1
fi
...
if [ -z "\$my_password" ]; then
    osascript -e 'display alert "Password cannot be empty. \nExiting." as critical'
    exit 1
fi
...
osascript -e 'display dialog "SSH keys generated and encrypted. \nDo you want to add them to the SSH agent?" buttons {"Yes", "No"} default button "Yes"'
...
if [ $? -eq 0 ]; then
    ...
    osascript -e 'display alert "SSH keys added to SSH agent."'
fi

# -------------------------------------------------------------------------- #

osascript -e 'display alert "Script finished."'
