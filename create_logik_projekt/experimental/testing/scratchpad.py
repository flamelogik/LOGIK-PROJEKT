...
class FlameWindow(QtWidgets.QWidget):
    ...
    FlameWindow(
        ...
        window_layout,
        window_width,
        window_height[, window_bar_color='blue']
    )

    window_title:   text displayed in top left corner of window [str]
    window_layout:  QLayout object.
                    QLayout should be defined before adding FlameWindow [object]

    window_width:   width of window [int]
    window_height:  height of window [int]
    window_bar_color: (optional) color of bar on left side of window.

        options are: red, blue, green, yellow, gray, teal. [str] - default is blue

    Usage:

        grid_layout = QtWidgets.QGridLayout()
        self.window = FlameWindow(
                        f'Import Camera <small>{VERSION}',
                        grid_layout, 400, 200
                    )
    '''
    ...
    def __init__(
            ...
            window_width: int,
            window_height: int,
            window_bar_color: Optional[str]='blue'
        ):
        ...
        if not isinstance(window_title, str):
            raise TypeError(
                'FlameWindow: Window Title must be a string.'
                )
        if not isinstance(window_width, int):
            raise TypeError(
                'FlameWindow: Window Width must be a string.'
                )
        ...
        if window_bar_color not in ['blue', 'red', 'green', 'yellow', 'gray', 'teal']:
           raise ValueError(
               'FlameWindow: Window Bar Color must be one of: blue, red, green, yellow, gray, teal.'
               )
        ...
        self.setStyleSheet(
            ...
            'QTabBar::tab:selected {'
                ...
                'border-bottom: 1px solid rgb(51, 102, 173)'
            '}'
            'QTabBar::tab:!selected {'
                'color: rgb(186, 186, 186); '
                'background-color: rgb(36, 36, 36); '
