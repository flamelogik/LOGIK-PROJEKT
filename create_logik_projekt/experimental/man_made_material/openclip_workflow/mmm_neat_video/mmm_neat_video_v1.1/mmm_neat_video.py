...
Menus:
    ...
    Config:
        ...
        Flame 2023.2+:
            ...
            Flame Main Menu -> mmm_python -> mmm_openclip -> configure mmm_neat_video

    Batch:

        Flame 2023.2+:

            Right-click on any clip in batch -> mmm_neat_video selected clips

    Media Panel:

        Flame 2023.2+:

            Right-click on any clip in media panel -> mmm_neat_video selected clips

To install:

    Copy script into /opt/Autodesk/shared/python/man_made_material/openclip_workflow/mmm_neat_video
...
from pyflame_lib_mmm_neat_video import (
    FlameMessageWindow,
    FlameWindow,
    FlameButton,
    FlameLabel,
...
class class_mmm_neat_video():
    ...
    def batch_mmm_neat_video_clips(self):
        import flame
    ...
    def media_panel_mmm_neat_video_clips(self):
        import flame

        flame.go_to('Batch')

        # Create batch group
        batch_group = flame.batch.create_batch_group('mmm_neat_video', reels=['sources','reference','CGI','mattes','neat_video','precomp','roto','comp'])
        ...
        self.batch_mmm_neat_video_clips()
    ...
    def create_batch_nodes(self, clip):
        ...
        def add_render_node():
            ...
            # add version note
            self.render_node.note = "This node was configured by mmm_neat_video."
        ...
        pyflame_print(SCRIPT_NAME, f'Added neat_video nodes added for: {self.clip_name}')
...
def mmm_neat_video_media_panel_clips(selection):

    script = class_mmm_neat_video(selection)
    script.media_panel_mmm_neat_video_clips()

def mmm_neat_video_batch_clips(selection):

    script = class_mmm_neat_video(selection)
    script.batch_mmm_neat_video_clips()
...
def get_batch_custom_ui_actions():
    ...
    return [
        ...
        },
        {
            ...
            'actions': [
               {
                    'name': 'mmm_neat_video selected clips',
                    'order': 3,
                    'separator': 'below',
                    'isVisible': scope_clip,
                    'execute': mmm_neat_video_batch_clips,
                    'minimumVersion': '2025'
               }
           ]
        }
            ...
            'actions': [
               {
                    ...
                    'separator': 'below',
                    'isVisible': scope_clip,
                    'execute': mmm_neat_video_media_panel_clips,
                    'minimumVersion': '2025'
               }
           ]
        }
