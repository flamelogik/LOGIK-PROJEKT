...
Menus:
    ...
    Config:
        ...
        Flame 2023.2+:
            ...
            Flame Main Menu -> mmm_python -> mmm_openclip -> configure mmm_comp

    Batch:

        Flame 2023.2+:

            Right-click on any clip in batch -> mmm_comp selected clips

    Media Panel:

        Flame 2023.2+:

            Right-click on any clip in media panel -> mmm_comp selected clips
...
from pyflame_lib_mmm_comp import (
    FlameMessageWindow,
    FlameWindow,
    FlameButton,
    FlameLabel,
...
def mmm_comp_media_panel_clips(selection):

    script = class_mmm_comp(selection)
    script.media_panel_mmm_comp_clips()

def mmm_comp_batch_clips(selection):
            ...
            'actions': [
                {
                    ...
                    'separator': 'below',
                    'isVisible': scope_clip,
                    'execute': mmm_comp_media_panel_clips,
                    'minimumVersion': '2025'
                }
            ]
        }
