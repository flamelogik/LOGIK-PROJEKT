...
proj_res_h = None
proj_res_v = None
proj_aspect_ratio = None
proj_color_depth = None
proj_fcm = None
proj_color_science = None
...
#     resolution_label.setText(f"Resolution:    {proj_res_h} x {proj_res_v}")
#     # aspect_ratio_label.setText(f"Aspect Ratio:  {proj_aspect_ratio}")
#     color_depth_label.setText(f"Color Depth:   {proj_color_depth}")
#     frame_rate_label.setText(f"Frame Rate:    {proj_fcm}")
#     color_science_label.setText(f"Color Science: {proj_color_science}")
...
#     summary_textbox.append(f"{proj_res}")
#     summary_textbox.append(f"{proj_aspect_ratio}")
#     summary_textbox.append(f"{proj_color_depth}")
#     summary_textbox.append(f"{proj_fcm}")
#     summary_textbox.append(f"{proj_color_science}")
...
def update_summary():
    ...
    try:
        ...
        proj_res = f"{proj_res_h} x {proj_res_v}"
        proj_aspect_ratio = float(proj_res_h) / float(proj_res_v)
        proj_aspect_ratio = round(proj_aspect_ratio, 2)  # Round up to two decimal places
    except ValueError:
        proj_res = "Invalid resolution"
        proj_aspect_ratio = "N/A"
    ...
    client_label.setText(f"Client Name:   {client_name_entry.text()}")
    campaign_label.setText(f"Campaign Name: {campaign_name_entry.text()}")
    resolution_label.setText(f"Resolution:    {proj_res}")
    aspect_ratio_label.setText(f"Aspect Ratio:  {proj_aspect_ratio}")
    color_depth_label.setText(f"Color Depth:   {proj_color_depth}")
    frame_rate_label.setText(f"Frame Rate:    {proj_fcm}")
    color_science_label.setText(f"Color Science: {proj_color_science}")
...
def build_app():
    ...
    client_label.setStyleSheet("color: #CCCCCC; font-weight: bold;")  # Slightly less than white
    ...
    campaign_label.setStyleSheet("color: #CCCCCC; font-weight: bold;")  # Slightly less than white
    ...
    aspect_ratio_label.setStyleSheet("color: #CCCCCC; font-weight: bold;")
    # layout.addWidget(aspect_ratio_label)

    # Initialize color_depth_label
    color_depth_label = QLabel("Color Depth: N/A")
    color_depth_label.setStyleSheet("color: #CCCCCC; font-weight: bold;")
