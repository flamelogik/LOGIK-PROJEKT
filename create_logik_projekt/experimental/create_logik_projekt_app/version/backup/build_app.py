...
# def create_popup_menu(items, label_text, separator_indices=None):
#     label = QLabel(label_text)
#     label.setStyleSheet("color: #CCCCCC; font-weight: bold;")  # Slightly less than white
...
#         if separator_indices and index in separator_indices:
#             popup_menu.insertSeparator(popup_menu.count())
...
def create_popup_menu(items, label_text):
    ...
    popup_menu = SeparatorComboBox()

    separator_indices = list(range(len(items) - 1))
    for index, item in enumerate(items):
        separator = index in separator_indices
        popup_menu.addItem(item, separator)
...
# Pop-Up Menus with separators
    ...
    generate_button.setStyleSheet("background-color: #600606; color: white; font-weight: bold;")
