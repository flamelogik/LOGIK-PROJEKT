...
Menus:
    ...
    Config:
        ...
        Flame 2023.2+:
            ...
            Flame Main Menu -> mmm_python -> mmm_openclip -> configure mmm_precomp

    Batch:

        Flame 2023.2+:

            Right-click on any clip in batch -> mmm_precomp selected clips

    Media Panel:

        Flame 2023.2+:

            Right-click on any clip in media panel -> mmm_precomp selected clips

To install:

    Copy script into /opt/Autodesk/shared/python/man_made_material/openclip_workflow/mmm_precomp
...
from pyflame_lib_mmm_precomp import (
    FlameMessageWindow,
    FlameWindow,
    FlameButton,
    FlameLabel,
...
class class_mmm_precomp():
    ...
    def batch_mmm_precomp_clips(self):
        import flame
    ...
    def media_panel_mmm_precomp_clips(self):
        import flame

        flame.go_to('Batch')

        # Create batch group
        batch_group = flame.batch.create_batch_group('mmm_precomp', reels=['sources','reference','CGI','mattes','neat_video','precomp','roto','comp'])
    ...
    def create_batch_nodes(self, clip):
        ...
        def add_render_node():
            ...
            # add version note
            self.render_node.note = "This node was configured by mmm_precomp."
...
def mmm_precomp_media_panel_clips(selection):

    script = class_mmm_precomp(selection)
    script.media_panel_mmm_precomp_clips()

def mmm_precomp_batch_clips(selection):

    script = class_mmm_precomp(selection)
    script.batch_mmm_precomp_clips()
...
def get_batch_custom_ui_actions():
    ...
    return [
        ...
        },
        {
            ...
            'actions': [
               {
                    'name': 'mmm_precomp selected clips',
                    'order': 1,
                    'separator': 'below',
                    'isVisible': scope_clip,
                    'execute': mmm_precomp_batch_clips,
            ...
            'actions': [
               {
                    ...
                    'separator': 'below',
                    'isVisible': scope_clip,
                    'execute': mmm_precomp_media_panel_clips,
                    'minimumVersion': '2023.2'
               }
           ]
        }
